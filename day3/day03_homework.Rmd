...
class(my.event.data$type_of_violence)
...
```{r}
my.event.data$factor_violencetype<-factor(my.event.data$type_of_violence, levels = c(1,2,3), labels = c("state-based conflict", "non-state conflict", "one-sided violence"))
...
xtabs(~ factor_violencetype + relevelfunc, my.event.data)
...
```{r}
my.event.data[!complete.cases(my.event.data),] #Yes, there are 7060 missing values (NA) in my dataset
